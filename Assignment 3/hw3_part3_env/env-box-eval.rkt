...
(define thunk? 
  (lambda (x) (and (list? x) (= (length x) 3) (eq? (car x) 'thunk))))
(define thunk->exp
  (lambda (thunk) (second thunk)))
(define thunk->env
  (lambda (thunk) (third thunk)))
  ...
  (lambda (exp env)
    (cond ((num-exp? exp) (num-exp->val exp))
          ...
          ((var-ref? exp)
           ...
           (if (thunk? val)
               (env-box-eval (thunk->exp val) (thunk->env val))
           val)))
          ...
          ;; APP-EXP
          ((app-exp? exp)
           (let ((proc (env-box-eval (app-exp->rator exp) env)))
             (env-box-apply-procedure proc
                                    (map (lambda (rand)
                                            (if (and (closure? proc)
                                             (var-lazy-decl? (list-ref (closure->params proc) (index-of (app-exp->rands exp) rand))))
                                      (make-thunk rand env)
                                      (env-box-eval rand env)))
                                         (app-exp->rands exp)))))
