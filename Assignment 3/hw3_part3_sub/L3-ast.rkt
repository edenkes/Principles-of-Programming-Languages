...
(define make-var-lazy-decl
  (lambda (var) (list 'var-lazy-decl (first var))))
(define var-lazy-decl?
  (lambda (x) (and (list? x) (= (length x) 2) (eq? (car x) 'var-lazy-decl))))
(define var-lazy-decl->var
  (lambda (var-lazy-decl) (second var-lazy-decl)))

(define lazy? (lambda (exp) (and (pair? exp) (eq? (second exp) 'lazy))))

(define make-var-decl-or-lazy
  (lambda (var)
    (if (lazy? var) (make-var-lazy-decl var) (make-var-decl var))))
...
;(parseL3 '((lambda ((a lazy)) 1) (/ 1 0)))
;'(app-exp (proc-exp ((var-decl a)) ((num-exp 1))) ((app-exp (prim-op /) ((num-exp 1) (num-exp 0)))))
    ...
    (cond ((empty? x) (make-lit-exp x)) ; ##### L3
          ((list? x) 
           (cond ((eq? (car x) 'L3)
                 ...
                 ((eq? (car x) 'lambda)
                  (make-proc-exp (map make-var-decl-or-lazy (second x))
                                 (map parseL3 (cddr x))))
                 ((eq? (car x) 'quote)
