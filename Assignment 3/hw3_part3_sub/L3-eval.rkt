...
(define L3-applicative-eval 
  (lambda (exp env)
    ...
    (cond ((num-exp? exp) (num-exp->val exp))
          ...
          ((app-exp? exp)
           (let ((proc (L3-applicative-eval (app-exp->rator exp) env)))
             (L3-apply-procedure proc
                                    (map (lambda (rand)
                                            (if (and (closure? proc)
                                             (var-lazy-decl? (list-ref (closure->params proc) (index-of (app-exp->rands exp) rand))))
                                      rand
                                      (L3-applicative-eval rand env)))
                                         (app-exp->rands exp))env)))
    ...
    (cond ((prim-op? proc)
          ...
          ((closure? proc)
           (let ((vars (map var-decl->var (closure->params proc)))
                 ...
                 (subst (map list (closure->params proc) args)))
             (let ((lit-args (map (lambda(x) (if (var-lazy-decl? (first x)) (second x) (L3-value->lit-exp (second x)))) subst)))
  ...
  (lambda (body vars exps)
    (letrec ((sub (lambda (exp) 
                    (cond ((num-exp? exp) exp)
                          ...
                          ((app-exp? exp)
                           ;(if (eq? (var-ref->var (car(app-exp->rands exp))) 'lazy) (sub (app-exp->rator exp))
                    ...
                    ;; First recursively rename all proc-exps inside the body.
                     ...
                     (let ((old-params (map var-decl->var (proc-exp->params exp)))
                       ...
                       (let ((new-param-names (map gen-var old-params))
                         ...
                         (let ((subst (map list (proc-exp->params exp) new-param-names)))
                         (make-proc-exp (map (lambda(x) (if (var-lazy-decl? (first x)) (make-var-lazy-decl (list (second x) 'lazy)) (make-var-decl (second x)) )) subst)
