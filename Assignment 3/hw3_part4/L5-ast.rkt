...
;;quote-te
(define make-quote-te
  (lambda () 'quote-te))
(define quote-te?
  (lambda (ex) (eq? ex 'quote-te)))
...
(define parse-texp
  (lambda (texp) 
    (cond ((eq? texp 'number) (make-num-te))
          ...
          ((eq? texp 'Void) (make-void-te))
          ((eq? texp 'Symbol) (make-quote-te))
          ((eq? texp 'symbol) (make-quote-te))
          ((symbol? texp) (make-tvar texp))
          ((list? texp) 
           (cond ((or (eq? (car texp) 'Pair)
                      (eq? (car texp) 'pair))
                  (let ((t1 (parse-texp (second texp)))
                        (t2 (parse-texp (third texp))))
                    (make-pair-te t1 t2)))              
                 ;; expected structure: (<params> -> <returnte>)
                 ;; expected exactly one -> in the list
                 ;; We do not accept (a -> b -> c) - must parenthesize
                 (else (let-values (([params arrow-return-te]
  ...
  (lambda (te) 
    (letrec ((unparse-tuple (lambda (param-tes)
      ...
      (cond ((eq? te 'num-te) 'number)
            ...
            ((eq? te 'void-te) 'void)
            ((eq? te 'quote-te) 'symbol)
            ((pair-te? te) (list 'Pair (unparse-texp (pair-te->t1 te)) (unparse-texp (pair-te->t2 te))))
            ((tvar? te)
