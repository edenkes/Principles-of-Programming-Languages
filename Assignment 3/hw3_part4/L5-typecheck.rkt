...
(define typeof-exp
  (lambda (exp tenv)
    (cond ((num-exp? exp)    (typeof-num-exp exp))
          ...
          ((lit-exp? exp)    (typeof-lit-exp exp tenv))
...
;; Purpose: compute the type of an lit-exp
(define typeof-lit-exp
  (lambda (quote tenv)
    (cond ((list? (lit-exp->val quote))
           (cond ((= (length (lit-exp->val quote)) 0) 0)
                 ((= (length (lit-exp->val quote)) 1)
                  (typeof-exp (make-lit-exp (first (lit-exp->val quote))) tenv))
                 (else (make-pair-te
                        (typeof-exp (make-lit-exp (first (lit-exp->val quote))) tenv)
                        (typeof-exp (make-lit-exp (second (lit-exp->val quote))) tenv)))))
          ((number? (lit-exp->val quote)) (make-num-te))
          ((boolean? (lit-exp->val quote)) (make-bool-te))
          ((pair? (lit-exp->val quote))
           (make-pair-te
            (typeof-exp (make-lit-exp (car (lit-exp->val quote))) tenv)
            (typeof-exp (make-lit-exp (cdr (lit-exp->val quote))) tenv)))
          ((symbol? (lit-exp->val quote)) (make-quote-te))
...
;; Signature: typeof-def-exp(defexp, tenv)
;; Type: [Texp -> Texp]
(define typeof-def-exp
  (lambda (defexp tenv)
    (let ((ver-decl (third (car (cdr defexp))))
          (ver-ref  (typeof-exp (second (cdr defexp)) tenv)))
      (check-equal-type! ver-decl ver-ref defexp)
      'void-te)))
...
(define typeof-app-exp
  (lambda (app tenv)
    (let ((rator (app-exp->rator app))
      ...
      (cond ((eq? (second rator) 'cons) (make-pair-te (typeof-exp (first rands) tenv) (typeof-exp (second rands) tenv)))
            ((eq? (second rator) 'car) (pair-te->t1 (typeof-exp (first rands) tenv)) )
            ((eq? (second rator) 'cdr) (pair-te->t2 (typeof-exp (first rands) tenv)))
            (else (let ((rator-te (typeof-exp rator tenv)))
                    (if (not (= (length rands)
